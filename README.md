...
- `POST /cart` - adiciona produto ao carrinho da sessão
- `POST /cart/add_product` - alias retrocompatível para adição de produto
- `POST /cart/add_item` - incrementa quantidade de produto existente no carrinho
- `GET /cart` - lista produtos e total do carrinho atual
- `DELETE /cart/:product_id` - remove produto do carrinho
- `resources :products` - CRUD completo de produtos

### Regras de negócio implementadas
- Carrinho é criado automaticamente e vinculado à sessão quando necessário.
- `quantity` deve ser maior que zero para adição/alteração de itens.
...
Subir API, banco, Redis e Sidekiq:
```bash
